...
export async function sliceModel(file: Buffer, filename: string, settings: SlicingSettings, quoteId?: string): Promise<SliceResult> {
	...
	if (settings.printer && settings.preset) {
		...
		try {
			...
			throw new AppError(400, 'Printer not found', `Printer ${settings.printer} not found`);
		}
	...
	if (settings.filament) {
		...
		try {
			...
			throw new AppError(400, 'Filament not found', `Filament ${settings.filament} not found`);
		}
	}
	...
	try {
		...
		throw new AppError(500, 'Failed to slice the model', `OrcaSlicer error: ${err.stderr || err.stdout || err.message}`);
	}
	...
	if (resultFiles.length === 0) {
		await fs.rm(workdir, { recursive: true, force: true });
		throw new AppError(500, 'No output files generated by slicer');
	}
